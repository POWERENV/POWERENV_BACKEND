...
bool isPublishing = args.Contains("--operation") && args.Contains("publish");
...
builder.AddProject<Projects.POWERENV_BACKEND_API>("MAIN-API").WithReference(redisCache).WithContainerRegistry(registry);

builder.AddProject<Projects.POWERENV_OSCONSOLE_WORKER>("OSCONSOLE-WORKER").WithReference(redisCache).WithContainerRegistry(registry);

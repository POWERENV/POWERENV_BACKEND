...
CREATE OR REPLACE PROCEDURE SP_GET_USER_BATCH_OPERATION_LOGS (
    _targetUserID INTEGER,
    INOUT result_set REFCURSOR -- Add an INOUT parameter for the cursor
)
LANGUAGE plpgsql
AS $$
        ...
        OPEN result_set FOR
            (
                ...
                FROM PPOOLS_BATCH_OPERATIONS
                         ...
                         INNER JOIN USERS ON USERS.USER_ID = PPOOLS_BATCH_OPERATIONS.BATCH_OPERATION_SOURCE_USER_ID
                         INNER JOIN PGRIDS ON PGRIDS.PGRID_ID = PPOOLS.ppool_associaterd_pgrid_id
                WHERE PGRIDS.PGRID_OWNER_ID = _targetUserID
                ORDER BY PPOOLS_BATCH_OPERATIONS.BATCH_OPERATION_ID DESC
            )
            UNION ALL
            (
                WITH SHARED_PGRIDS_CTE AS (
                    SELECT PGRID_ID AS SHARED_PGRID_ID
                    FROM PGRID_ACCESS_POLICIES
                    INNER JOIN PGRIDS ON pgrid_id = pgrid_access_policies.access_policy_pgrid_id
                    WHERE PGRID_ACCESS_POLICIES.access_policy_target_user_id = _targetUserID
                )
                SELECT PPOOLS_BATCH_OPERATIONS.BATCH_OPERATION_ID,
                   PNODE_OPERATION_CATEGORIES.OPERATION_CAT_NAME,
                   PPOOLS_BATCH_OPERATIONS.BATCH_OPERATION_SOURCE_PPOOL_ID,
                ...
                FROM PPOOLS_BATCH_OPERATIONS
                         ...
                         INNER JOIN USERS ON USERS.USER_ID = PPOOLS_BATCH_OPERATIONS.BATCH_OPERATION_SOURCE_USER_ID
                         INNER JOIN PGRIDS ON PGRIDS.PGRID_ID = PPOOLS.ppool_associaterd_pgrid_id
                WHERE PGRIDS.PGRID_ID IN (SELECT SHARED_PGRIDS_CTE.SHARED_PGRID_ID FROM SHARED_PGRIDS_CTE)
                ORDER BY PPOOLS_BATCH_OPERATIONS.BATCH_OPERATION_ID DESC
            );
    END;
$$;
